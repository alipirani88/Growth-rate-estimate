...
###Pending changes
def generate_perc_coverage_graph(csv_matrix_file, ptr_value, analysis):
    out_path = os.path.dirname(csv_matrix_file)
    header = os.path.basename(csv_matrix_file).replace('.csv', '')
    with open(out_path + "/%s_perc_coverage_graph.R" % analysis, 'w') as out:
        ...
        print_string = print_string + "pdf(\"%s_perc_coverage_graph.pdf\", width = 15, height = 10)\n" % analysis
