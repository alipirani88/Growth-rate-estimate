...
def smoothing_1(read_counts, window, out_path, bedfile, logger, Config):
    keep_logging('Smoothing data frame for Growth Rate analysis', 'Smoothing data frame for Growth Rate analysis',
                 logger, 'info')
    ...
    with open(out_file_ptr, 'a') as out:
        ...
        out.write(
            str(out_path) + ' Peak and trough location:\t' + str(median_sliding_window_array.index(peak)) + '\t' + str(
                median_sliding_window_array.index(through)) + '\n')
...
def generate_PTR_dataframe(bedfile, out_path, analysis, logger, Config):
    keep_logging('Reading BED file: {} and generating read counts matrix'.format(bedfile),
                 'Reading BED file: {} and generating read count matrix'.format(bedfile), logger, 'info')
